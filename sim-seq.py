...
# The name of the file created is made unique by adding a timestamp to it.
# 
...
#Obtain timestamp to use it to uniquely identify output files
...
if mode == 'se':
	...
	output = open(output_folder + '/se_reads_' + timestamp + '.fq', 'w')
	...
	for genome_index, template_genome in enumerate(template_genomes):		
		...
		while read_count <= number_of_reads_per_genome:
			...
			output.write('@' + timestamp + '-gn' + str(genome_index + 1) + '-rd' + str(read_count) + '\n' + read_seq + '\n+\n' + qual_seq + '\n')
...
if mode == 'pe':
	...
	output_for = open(output_folder + '/pe-for_reads_' + timestamp + '.fq', 'w')
	output_rev = open(output_folder + '/pe-rev_reads_' + timestamp + '.fq', 'w')
	...
	for genome_index, template_genome in enumerate(template_genomes):		
		...
		while fragment_count <= number_of_reads_per_genome / 2:
			...
			output_for.write('@' + timestamp + '-gn' + str(genome_index + 1) + '-rd' + str(fragment_count) + '/1\n' + read_for_seq + '\n+\n' + qual_for_seq + '\n')
			output_rev.write('@' + timestamp + '-gn' + str(genome_index + 1) + '-rd' + str(fragment_count) + '/2\n' + read_rev_seq + '\n+\n' + qual_rev_seq + '\n')
